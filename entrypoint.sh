...
cp /code/codetypo-validator.json /github/workflow/codetypo-validator.json
echo "::add-matcher::${RUNNER_TEMP}/_github_workflow/codetypo-validator.json"

# Run codetypo.
# The weird piping was needed because we want to get the exitcode from codetypo,
# but there is no other good universal way of doing so.
# e.g. PIPESTATUS and pipestatus only work in bash/zsh respectively.
echo "Running codetypo on '${INPUT_PATH}' with the following options..."
command_args=""
...
echo "::remove-matcher owner=codetypo-validator-default::"
echo "::remove-matcher owner=codetypo-validator-specified::"

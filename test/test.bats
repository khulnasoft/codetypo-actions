...
function setup() {
    ...
    [ -f "${RUNNER_TEMP}/code/codetypo-validator.json" ] || sudo cp codetypo-validator/codetypo-validator.json ${RUNNER_TEMP}/code/
    #ls -alR ${RUNNER_TEMP}/code/
    [ -d "/code/" ] || sudo mkdir -p /code/
    [ -f "/code/codetypo-validator.json" ] || sudo cp codetypo-validator/codetypo-validator.json /code/
    #ls -alR /code/
...
@test "Run with defaults" {
    ...
    # codetypo's exit status is 0, or 65 if there are errors found
    if [ $errorCount -eq 0 ]; then expectedExitStatus=0; else expectedExitStatus=65; fi
    run "./entrypoint.sh"
    [ $status -eq $expectedExitStatus ]

    # Check output
    [ "${lines[0]}" == "::add-matcher::${RUNNER_TEMP}/_github_workflow/codetypo-validator.json" ]
